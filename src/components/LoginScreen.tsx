...
const DEMO_USERS: Array<UserProfile & { password: string }> = [
  {
    id: 'usr_demo_asn',
    name: 'Dr. Raden Triyono, M.T.',
    email: 'triyono@komdigi.go.id',
    nik: '198204152006041002',
    role: 'ASN Kominfo RI',
  ...
  },
  {
    ...
    nik: '3201025508980003',
    role: 'Masyarakat Umum / Professional',
...
function getLocalDemoUser(identity: string, password: string): UserProfile | null {
  const cleanIdentity = identity.trim().toLowerCase();
  const match = DEMO_USERS.find((user) =>
    (user.email.toLowerCase() === cleanIdentity || user.nik === cleanIdentity || (cleanIdentity === 'demo' && user.id === 'usr_demo_asn'))
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Akun uji coba tetap dapat membuka dashboard saat Vercel Function belum aktif.
      const localDemoUser = getLocalDemoUser(identity, password);
      if (localDemoUser) {
        setSuccessMsg('Mode demo lokal aktif. Membuka Dasbor Portal...');
        setTimeout(() => {
          onLoginSuccess(localDemoUser, `demo_local_${localDemoUser.id}_${Date.now()}`);
        }, 350);
      } else {
